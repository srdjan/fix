...
export const dbMacro: Macro<Meta, { db: DbPort } & { lease: Pick<LeasePort<any>, "db" | "tx"> }> = ({
  key: "db",
  match: (m) => !!m.db,
...
export const socketMacro: Macro<Meta, { lease: Pick<LeasePort<any>, "socket"> }> = ({
  key: "socket",
  match: (m) => !!m.socket,
  resolve: async (_m, env: any) => ({ lease: { socket: env.makeSocket() } as any })
});
...
export const idempotencyMacro: Macro<Meta, {}> = ({
  ...
  before: async (ctx: any) => {
    const key: string | undefined = ctx?.meta?.idempotency?.key || ctx?.idempotencyKey;
    if (key && ctx.kv) {
      const val = await ctx.kv.get<any>(`idem:${key}`);
      if (val) {
        ctx.log?.info?.("idempotency.hit", { key });
        // signal to executor? simplest: stash in ctx
        ctx.__idemReturn = val;
      }
    }
  },
  after: async (value: any, ctx: any) => {
    const key: string | undefined = ctx?.meta?.idempotency?.key || ctx?.idempotencyKey;
    if (key && ctx.kv && ctx.__idemReturn === undefined) {
      await ctx.kv.set(`idem:${key}`, value, ctx.meta?.idempotency?.ttlMs ?? 5 * 60_000);
    }
    return ctx.__idemReturn ?? value;
  }
});

export const stdMacros = [
  httpMacro, kvMacro, dbMacro, queueMacro, timeMacro, cryptoMacro, logMacro,
  fsMacro, lockMacro, socketMacro, idempotencyMacro
] as const;
