...
import type { DbPort, HttpPort, KvPort, LeasePort, LogLevel, LogPort, QueuePort, TimePort } from "../ports/mod.ts";
import { makePool } from "../resources/pool.ts";

// --- HTTP (fetch-based) ---
export function makeHttp(baseUrl = "", _opts?: { auth?: "bearer" | "none" }): HttpPort {
  const toURL = (p: string) => new URL(p, baseUrl || "http://localhost");
  return {
    get: (p, init) => fetch(toURL(p), { ...init, method: "GET" }),
    post: (p, body, init) => fetch(toURL(p), {
      ...init,
      method: "POST",
...
export function makeKv(namespace: string): KvPort {
  ...
  return {
    async get<T>(key: string) {
      ...
      if (!v) return null;
      if (v.exp && v.exp < Date.now()) { store.delete(k(key)); return null; }
      return v.value as T;
    },
    async set<T>(key: string, value: T, ttlMs?: number) {
      store.set(k(key), { value, exp: ttlMs ? Date.now() + ttlMs : undefined });
    },
    async del(key: string) { store.delete(k(key)); }
  };
}
...
export function makeDb(meta: { role: "ro" | "rw" }) {
  ...
  async function tx<T>(fn: (db: any) => Promise<T>): Promise<T> {
    ...
    try {
      ...
      try { await db.rollback(); } finally { await pool.release(db); }
      throw e;
    }
  }
  ...
  const leaseDb = async <S>(_role: "ro" | "rw") => {
    const db = await pool.acquire();
    return { value: db, release: async () => { await pool.release(db); } };
  };
...
export function makeQueue(name: string): QueuePort {
  return { async enqueue(msg) { console.log(`[queue:${name}]`, msg); } };
}
...
export function makeLogger(level: LogLevel = "info"): LogPort {
  ...
  return { level, debug: log("debug"), info: log("info"), warn: log("warn"), error: log("error") };
}
...
export const hostNodeEnv = {
  ...
  makeLogger,
  fs,
  makeLock: () => async (key: string) => ({ value: { key }, release: async () => {} }),
  makeSocket: () => async (_host: string, _port: number) => ({ value: { write: async () => {}, close: async () => {} }, release: async () => {} })
};
